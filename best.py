...
with st.sidebar:
    ...
    with col1:
        if st.button("📊 2013", help="Full year 2013"):
            start_date = dt.date(2013, 1, 1)
            end_date = dt.date(2013, 12, 31)
            st.rerun()

    with col2:
        if st.button("📊 2016", help="Full year 2016"):
    ...
    - **Profit**: Gross profit earned
    - **Margin**: Profit as % of sales
    - **COGS**: Cost of goods sold
    - **Coverage**: Deal coverage percentage
    """)
...
<div class="main-header">
    ...
    <p>Comprehensive insights into sales, procurement, and supply chain performance</p>
</div>
""", unsafe_allow_html=True)
...
with fin_col4:
    st.metric(
        label="📊 Gross Margin",
        value=f"{margin:.1f}%",
        delta="Target: 25%" if margin < 25 else "Above Target",
        delta_color="inverse" if margin < 25 else "normal",
...
with promo_col1:
    st.metric(
        label="📈 Deal Coverage",
        value=f"{cov:.1f}%",
        delta="Target: 80%" if cov < 80 else "Above Target",
        delta_color="inverse" if cov < 80 else "normal",
...
st.markdown("---")
st.markdown("""
<div style="text-align: center; padding: 2rem; background: #0000; border-radius: 10px; margin-top: 2rem; margin-bottom: 2rem; border: 1px solid #e9ecef;">
    <h4>📊 Supply Chain Analytics Dashboard</h4>
    <p>Powered by SQL Server • Streamlit • Plotly</p>
    <p><small>© 2025 • Last Updated: {}</small></p>
    <p><small>Data Period: {} to {}</small></p>
</div>
""".format(
